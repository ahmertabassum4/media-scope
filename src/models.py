...
def aligned_scores(model, matrix):
...
def _tune(X, y, inner_splits, seed=3042):
    ...
    for spec in _SPECS:
        ...
        rep = classification_report(yy, preds, labels=CLASS_ORDER, zero_division=0, output_dict=True)
        key = (float(f1_score(yy, preds, average="macro", zero_division=0)),
               float(accuracy_score(yy, preds)),
               float(rep["MIXED"]["f1-score"]), spec)
        if best is None or key[:3] > best[:3]:
            best = key
    return dict(best[3])


def feature_expert(X_train, y, X_test, inner_splits, seed=4042):
    spec = _tune(X_train, y, inner_splits)
    cv = StratifiedKFold(n_splits=inner_splits, shuffle=True, random_state=seed)
    yy = y.to_numpy()
    ...
    for fold, (tr, va) in enumerate(cv.split(X_train, y), start=1):
        ...
        oof[va] = aligned_scores(model, X_train.iloc[va])
    final = _estimator(spec, seed + 999)
    final.fit(X_train, yy)
    return oof, aligned_scores(final, X_test)
...
def ocr_expert(train_text, test_text, y, inner_splits, seed=4542):
    yy = y.to_numpy()
    cv = StratifiedKFold(n_splits=inner_splits, shuffle=True, random_state=seed)
    oof = np.zeros((len(y), len(CLASS_ORDER)), dtype=float)
    ...
    clf.fit(Xfull, yy)
    return oof, aligned_scores(clf, Xtest)
...
def dino_expert(train_emb, test_emb, y, inner_splits, seed=5042):
    yy = y.to_numpy()
    cv = StratifiedKFold(n_splits=inner_splits, shuffle=True, random_state=seed)
    oof = np.zeros((len(y), len(CLASS_ORDER)), dtype=float)
    for fold, (tr, va) in enumerate(cv.split(train_emb, y), start=1):
        ...
        oof[va] = aligned_scores(model, train_emb.iloc[va])
    final = _dino_model(train_emb, seed + 9000)
    final.fit(train_emb, yy)
    return oof, aligned_scores(final, test_emb)


# ── balanced logistic-regression meta stack ──────────────────────────────────
def meta_predict(Z_train, y, Z_test, seed=6042):
    ...
    scores = aligned_scores(final, Z_test)
    return np.asarray(CLASS_ORDER, dtype=object)[np.argmax(scores, axis=1)]


def fit_config(name, data, inner_splits):
    use_features, use_ocr, use_dino = MODEL_CONFIGS[name]
    train_blocks, test_blocks = [], []
    if use_features:
        oof, test = feature_expert(data["X_train"], data["y_train"], data["X_test"], inner_splits)
        train_blocks.append(oof)
        test_blocks.append(test)
    if use_ocr:
        oof, test = ocr_expert(data["train_text"], data["test_text"], data["y_train"], inner_splits)
        train_blocks.append(oof)
        test_blocks.append(test)
    if use_dino:
        oof, test = dino_expert(data["train_emb"], data["test_emb"], data["y_train"], inner_splits)
        train_blocks.append(oof)
        test_blocks.append(test)
    return meta_predict(np.hstack(train_blocks), data["y_train"], np.hstack(test_blocks))

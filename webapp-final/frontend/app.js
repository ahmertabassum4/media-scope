...
const factualityMeme = document.getElementById("factualityMeme");
const memeVerdict = document.getElementById("memeVerdict");
...
// Map each factuality label to a meme: high/very high are trustworthy,
// low/very low are untrustworthy, and mixed gets its own meme.
const factualityMemes = {
  "very high": { src: "/memes/Trustworthy.png", verdict: "Trustworthy" },
  high: { src: "/memes/Trustworthy.png", verdict: "Trustworthy" },
  mixed: { src: "/memes/Mixed_Factuality.png", verdict: "Mixed" },
  low: { src: "/memes/Untrustworthy.png", verdict: "Untrustworthy" },
  "very low": { src: "/memes/Untrustworthy.png", verdict: "Untrustworthy" },
};
...
function renderMeme(factualityLabel) {
  const meme = factualityMemes[factualityLabel];
  if (!meme) {
    hideMeme();
    return;
  }
  factualityMeme.src = meme.src;
  factualityMeme.alt = `${meme.verdict} meme`;
  memeVerdict.textContent = meme.verdict;
  memeVerdict.style.color = labelColors.factuality[factualityLabel] || "";
  memeCard.hidden = false;
}

function hideMeme() {
  memeCard.hidden = true;
  factualityMeme.removeAttribute("src");
  factualityMeme.alt = "";
  memeVerdict.textContent = "";
}
...
function renderResult(data) {
  ...
  renderMeme(data.factuality.label);

...
async def analyze_image(file: UploadFile = File(...)) -> JSONResponse:
    ...
    files = {"file": (file.filename or "uploaded-image", payload, file.content_type or "application/octet-stream")}
    try:
        async with httpx.AsyncClient(timeout=settings.models_request_timeout_seconds) as client:
            response = await client.post(f"{settings.models_api_url}/predict", files=files)
